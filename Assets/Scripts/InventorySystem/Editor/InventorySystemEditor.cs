...
public class InventorySystemEditor : EditorWindow
{
    ...
    private EnumField itemTypeEnum;
    private Button itemTypeAll;
    ...
    public void CreateGUI()
    {
        ...
        // 物品类别
        itemTypeEnum = root.Q<EnumField>("item-type");
        itemTypeEnum.RegisterValueChangedCallback((x) => OnItemTypeSelect(x.newValue));
        itemTypeAll = root.Q<Button>("item-type-all");
        itemTypeAll.clicked += OnRefresh;
    ...
    private void OnItemTypeSelect(Enum newValue)
    {
        if (database != null)
        {
            itemListView.itemsSource = database.GetItemsByType((ItemType)newValue);
        }
    }
    #endregion
    ...
    public void OnRefresh()
    {
        ...
        if (database != null)
        {
            ...
            itemTypeEnum.SetValueWithoutNotify(ItemType.Default);
    ...
    private void OnExeclPathValueChange(string newValue)
    {
        setting.execlPath = newValue;
        AssetDatabase.SaveAssets();
    }

    private void OnDatabasePathValueChange(string newValue)
    {
        setting.newDatabasePath = newValue;
        AssetDatabase.SaveAssets();
    }
