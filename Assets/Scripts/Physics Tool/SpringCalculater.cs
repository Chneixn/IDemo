...
public class SpringCalculater
{
    ...
    [Header("Taget")]
    public Vector3 TargetPosition = Vector3.zero;  // 弹簧另一点的位置
    public Vector3 TargetVelocity = Vector3.zero;
    [Header("Self")]
    public Vector3 SelfPosition = Vector3.zero;    // 自身位置
    public Vector3 SelfVelocity = Vector3.zero; // 当前速度
    public float Mass = 1f;  // 质量
    public float Length = 5f;    // 弹簧需要维持的长度
    public float maxDistance = Mathf.Infinity;
    public float minDistance = 0f;
    public float UAF = 8f; // 无阻尼角频率 弹簧的震动频率
    public float DR = 0.8f;  // Damping Ratio (DR) 阻尼比

    public Vector3 gravity = new(0f, -9.81f, 0f);
    [Header("Result")]
    [SerializeField] private Vector3 value = Vector3.zero;  // 计算后当前应有的速度
